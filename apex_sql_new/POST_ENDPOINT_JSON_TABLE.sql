...
-- Version: Using JSON_TABLE (native Oracle, most reliable)
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('CLOB length: ' || DBMS_LOB.GETLENGTH(v_body_clob));

    -- Parse JSON using JSON_TABLE (native Oracle, very reliable)
    BEGIN
        SELECT
            jt.trip_id,
            jt.trip_date,
            jt.order_count,
            jt.printer_config_id,
            jt.printer_name,
            jt.orders_json
        INTO
            v_trip_id,
        ...
        FROM JSON_TABLE(
            ...
            '$'
            COLUMNS (
                trip_id VARCHAR2(200) PATH '$.tripId',
                trip_date VARCHAR2(50) PATH '$.tripDate',
                order_count NUMBER PATH '$.orderCount',
                printer_config_id NUMBER PATH '$.printerConfigId',
                printer_name VARCHAR2(500) PATH '$.printerName',
                orders_json CLOB FORMAT JSON PATH '$.orders'
            )
        ) jt;
