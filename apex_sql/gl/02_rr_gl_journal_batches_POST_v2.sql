...
-- Recreate with smart format detection
BEGIN
    ORDS.DEFINE_HANDLER(
        p_module_name    => 'rr.gl',
        p_pattern        => 'journalbatches',
...
BEGIN
    ...
    -- Check if this is Oracle Fusion format with "items" array
    v_has_items := JSON_VALUE(v_body, '$.items' RETURNING VARCHAR2 ERROR ON ERROR);

    IF v_has_items IS NOT NULL THEN
...
PROMPT ✅ Updated POST /rr/gl/journalbatches endpoint successfully!
